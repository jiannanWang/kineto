...
TEST_F(GenericActivityProfilerTest, NoDuplicateFlowIdsUnchanged) {
  ...
  cpuOps->addOpWithFlow(
      "fwd1", start_time_ns + 10, start_time_ns + 30, 1,
      /*flowId=*/10, /*flowType=*/kLinkFwdBwd, /*flowStart=*/true);

  // Forward op with flow ID 20
  cpuOps->addOpWithFlow(
      "fwd2", start_time_ns + 40, start_time_ns + 60, 2,
      /*flowId=*/20, /*flowType=*/kLinkFwdBwd, /*flowStart=*/true);

  // Backward op with flow ID 10 (unique - no duplicate)
  cpuOps->addOpWithFlow(
      "bwd1", start_time_ns + 70, start_time_ns + 100, 3,
      /*flowId=*/10, /*flowType=*/kLinkFwdBwd, /*flowStart=*/false);

  // Backward op with flow ID 20 (unique - no duplicate)
  cpuOps->addOpWithFlow(
      "bwd2", start_time_ns + 110, start_time_ns + 150, 4,
      /*flowId=*/20, /*flowType=*/kLinkFwdBwd, /*flowStart=*/false);
